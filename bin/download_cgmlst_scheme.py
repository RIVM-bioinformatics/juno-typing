...
class cgMLSTSchemes(base_juno_pipeline.helper_functions.JunoHelpers):
    ...
    def __init__(self, 
        ...
        self.genus_list = list(map(str.lower, genus_list))
        ...
        if some_genus_not_supported:
            warning_message = f'The following genus are currently not supported for cgMLST: {genus_not_found}. '\
                'The schemes for those genus/genera will not be downloaded'
            warnings.warn(self.error_formatter(warning_message))
        no_genus_supported = len(genus_to_download) == 0
        if no_genus_supported:
            error_message = 'None of the provided genera is supported for cgMLST. ' \
                f'The supported genera/species are: {self.schemes.keys()}'
            raise ValueError(self.error_formatter(error_message))

        downloaded_schemes = {genus:info for (genus, info) in \
                self.download_cgmlst_scheme(genus_list=genus_to_download)}
        self.schemes = {genus: {**self.schemes[genus], **downloaded_schemes[genus]} \
    ...
    def download_cgmlst_scheme(self, genus_list):
        for genus in genus_list:
            ...
            print(self.message_formatter(f'Collecting cgMLST scheme for {genus.title()} from {source.title()}...'))
...
if __name__ == '__main__':
    ...
    argument_parser.add_argument('-g','--genus', metavar='N', 
                        type=str, nargs='+',
    ...
    cgMLSTSchemes(threads=args.threads, genus_list=args.genus, 
