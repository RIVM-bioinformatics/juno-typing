...
class SerotyperMultireport():
    ...
    def __init__(self,
        ...
        assert all([file_.is_file() for file_ in self.input_files]), "One or more of the specified input files do not exist!"
...
class SeqSero2Multireport(SerotyperMultireport):
    ...
    def make_multireport(self):
        multireport = [self.extract_from_seqsero2_result(seqsero2_result) for seqsero2_result in self.input_files]
        multireport = multireport[0].append([multireport[i] for i in range(1, len(multireport))], 
                                                ignore_index=True)
...
class SerotypeFinderMultireport(SerotyperMultireport):
    ...
    def make_multireport(self):
        ...
        results_df['O type'] = [self.report_o_type(row) for index, row in results_df.iterrows()]
...
class ChooseMultireport():
    ...
    def __classify_serotyper_result_files(self):
        ...
        if number_of_expected_multireports > 1: 
            warn('More than one serotyper was run in the input samples and therefore multiple serotypers will be created.')
            multireport_files = [self.out_dir.joinpath(f'serotyper_multireport{n}.csv') for n in range(0,number_of_expected_multireports)]
    ...
    def make_serotyper_multireports(self):
        ...
        for serotyper_tool in self.input_files:
            ...
            if serotyper_tool == 'seqsero2':
                multireport = SeqSero2Multireport(input_files=self.input_files[serotyper_tool],
